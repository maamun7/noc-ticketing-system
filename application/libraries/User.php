...
class User {
	...
	public function generate_list($limit,$page,$links)
	{
		...
		if(!empty($list)){		
			foreach($list as $k=>$v){$i++;
			   ...
			   if($v['user_id'] != 1 ){
					$list[$k]['can_edit']="<a class='btn btn-sm btn-success' href='" .base_url(). "user/edit/". $v['user_id'] ."'><i class='glyphicon glyphicon-edit glyphicon-white'></i></i></a>";
			   }else{
					$list[$k]['can_edit']="---";
			   }
			}
		...
		$list_view = $CI->parser->parse('admin/user/index',$data,true);
		return $list_view;
	}
	...
	public function new_user()
	{
		...
		$productForm = $CI->parser->parse('admin/user/add',$data,true);
		return $productForm;
	}
	...
	public function get_edit_data($user_id)
	{
		...
		$profile_data = $CI->parser->parse('admin/user/edit',$data,true);
		return $profile_data;
	}
	...
	public function edit_profile_form()
	{
		...
		$data = array(
				...
				'designation' => $edit_data[0]['designition'],
				'address' => $edit_data[0]['address']
			);	
		$profile_data = $CI->parser->parse('admin/user/edit_profile',$data,true);
		return $profile_data;
	}
}
?>