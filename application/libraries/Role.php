...
class Role {
	public function generate_role_list()
	{
		...
		$list_view = $CI->parser->parse('admin/roles/index',$data,true);
		return $list_view;
	}
	...
	public function new_role()
	{
		...
		$roleForm = $CI->parser->parse('admin/roles/add',$data,true);
		return $roleForm;
	}
	...
	public function role_edit_form($role_id)
	{
		...
		$roleEditForm = $CI->parser->parse('admin/roles/edit',$data,true);
		return $roleEditForm;
	} 
	...
	private function all_permission()
    {
      ...
      $permissions = $CI->parser->parse('admin/roles/all_permissions', array('groups'=>$custom), TRUE);
      return $permissions;
    }
	...
	public function permission_form($role_id)
	{
        ...
        $content = $CI->parser->parse('admin/roles/permission', $cdata, TRUE);
        return $content;
	}
