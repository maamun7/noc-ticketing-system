...
class Auth {
    ...
    // if user has desired task permission
    function check_adv_permission($slug, $user_id = 0)
    {
        $CI =& get_instance();
        $CI->load->model('users');
        if ( $user_id == 0 )
    ...
    function echeck_user_permission($user_id = 0)
    {
        $CI =& get_instance();
		$CI->load->model('users');
        if ( $user_id == 0 )
    ...
    public function manually_generate_session($user_id = false,$username,$user_type)
	{
		$CI =& get_instance();
